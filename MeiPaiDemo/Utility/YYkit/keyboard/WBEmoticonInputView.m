...
- (void)updateContent {
    ...
    if (_isDelete) {
        ...
        if (_emoticon.type == WBEmoticonTypeEmoji) {
            ...
            if (!pngPath) {
                NSString *addBundlePath = [[WBStatusHelper emoticonBundle].bundlePath stringByAppendingPathComponent:@"com.famenapp.emotion"];
            ...
            if (pngPath) {
                [_imageView yy_setImageWithURL:[NSURL fileURLWithPath:pngPath] options:YYWebImageOptionIgnoreDiskCache];
            }
        }
    }
}
...
- (void)_initToolbar {
    ...
    sendButton.frame = CGRectMake(kScreenWidth - 55, 0 , 55, kToolbarHeight);
...
- (UIButton *)_createToolbarButton {
    ...
    UIImage *img;
    img = [UIImage imageNamed:@"emojibtn_normal"];
    ...
    img = [UIImage imageNamed:@"emojibtn_focused"];
    img = [img resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, img.size.width - 1) resizingMode:UIImageResizingModeStretch];
...
- (void)sendButtonClicked:(UIButton*)btn{
    ...
    if ([self.delegate respondsToSelector:@selector(emoticonInputDidTapSendButton)]) {
