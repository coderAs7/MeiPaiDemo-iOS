    ...
    if (toDisk) {//图片是否存储到disk中
        dispatch_async(self.ioQueue, ^{
            ...
            if (image && (recalculate || !data)) {
                ...
                // 如果imageData为nil（也就是说，如果试图直接保存一个UIImage或者图片是由下载转换得来）并且图片有alpha通道，我们将认为它是PNG文件以避免丢失透明度信息。 an alpha channel, we will consider it PNG to avoid losing the transparency
...
/**如果此方法返回nil，则说明缓存中现在还没有这张照片，因此你需要得到并缓存这张图片。缓存key是缓存图片的程序唯一的标识符，一般使用图片的完整URL*/
