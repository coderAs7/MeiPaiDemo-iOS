...
@property (nonatomic, weak) MPBadgeView *badgeView;
...
// 懒加载badgeView
- (MPBadgeView *)badgeView{
    
    if (_badgeView == nil) {
        
        MPBadgeView *btn = [MPBadgeView buttonWithType:UIButtonTypeCustom];
...
- (void)layoutSubviews{
    ...
    // 3.badgeView
    self.badgeView.frame = CGRectMake((self.bounds.size.width - self.badgeView.bounds.size.width - 10), 0, self.badgeView.frame.size.width, self.badgeView.frame.size.height);
}
